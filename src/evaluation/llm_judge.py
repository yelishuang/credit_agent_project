...
RESPONSES_DIR = PROJECT_ROOT / "outputs" / "evaluation"
REPORT_PATH = RESPONSES_DIR / "eval_report.json"

BASE_MODEL_PATH = str(PROJECT_ROOT / "data" / "base_models" / "Qwen2.5-14B-Instruct")
LORA_ADAPTER_PATH = None  # run 模式必须通过 --lora-adapter 显式指定
LORA_SEARCH_ROOT = PROJECT_ROOT / "outputs" / "qwen2.5_14b_lora"
...
def get_eval_dir(adapter_path: str) -> Path:
    ...
    if parent_name != "qwen2.5_14b_lora":
...
def run_pairwise_evaluation(test_cases: list[dict], responses: dict,
                            judge_results_path: Path) -> list[dict]:
    """对所有测试用例进行 Pairwise 评测（含位置交换）。"""
    logger.info("=== Phase 3: Pairwise LLM Judge ===")
    results = []
    ...
    for i, tc in enumerate(test_cases):
        ...
        logger.info(f"Judging {i+1}/{len(test_cases)}: {tc['id']}")
...
def run_single_adapter(test_cases: list[dict], adapter_path: str, dry_run: bool = False):
    ...
    judge_results = []
    if not dry_run:
        judge_results = run_pairwise_evaluation(test_cases, responses, judge_results_path)
...
def main():
    ...
    parser.add_argument("mode", choices=["generate", "evaluate", "report", "run", "auto"],
                        help="generate/evaluate/report/run=指定adapter, auto=自动发现并测试未评测的adapter")
